...
->Changes by Nick Maliszewskyj/NIST     04/10/2020
...
{ 
  ...
  int bufsize;
	...
	cbuf = (unsigned char *)DataBuffer;
	usbuf = (unsigned short *)DataBuffer;
  cbuf[0]=12;
  cbuf[1]=0;
	bufsize = 2*(usbuf[1]+0x10000*usbuf[2])+4;
  libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_OUT, cbuf, bufsize, &transferred, timeout);
  if (transferred>0)
    libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_IN, cbuf, lDataLen, &transferred, timeout);
	
  return transferred;
}


int  ccusb_longstack_execute(libusb_device_handle *hDev, void *DataBuffer, int lDataLen, int timeout)
{ 
    int transferred;
    unsigned char *cbuf;
	  unsigned short *usbuf;
...
int  xxusb_bulk_write(libusb_device_handle *hDev, void *DataBuffer, int lDataLen, int timeout)
{
  ...
  unsigned char *cbuf;
  cbuf = (unsigned char *)DataBuffer;
  libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_OUT, cbuf, lDataLen, &transferred, timeout);
  return transferred;
}
...
int  xxusb_usbfifo_read(libusb_device_handle *hDev, int *DataBuffer, int lDataLen, int timeout)
{
  ...
  libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_IN, cbuf, lDataLen, &ret, timeout);
  if (ret > 0)
...
*/
short  xxusb_stack_write(libusb_device_handle *hDev, short StackAddr, long *intbuf)
{ 
  int timeout;
    int ret;
    short lDataLen;
    ...
    buf[0]=(unsigned char)((StackAddr & 51) + 4);
    buf[1]=0;
    lDataLen=(short)(intbuf[0] & 0xFFF);
    buf[2]=(unsigned char)(lDataLen & 255);
    lDataLen = lDataLen >> 8;
    buf[3] = (unsigned char)(lDataLen & 255);
    bufsize=intbuf[0]*2+4;
    if (intbuf[0]==0)
      return 0;
    for (i=1; i <= intbuf[0]; i++)
      {
        buf[2+2*i] = (unsigned char)(intbuf[i] & 255);
        buf[3+2*i] = (unsigned char)((intbuf[i] >>8) & 255);
      }
    timeout=100;
    libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_OUT, buf, bufsize, &ret, timeout);
    return (short) ret;
}
...
******** xxusb_stack_execute **********************
  ...
  Returns:
    ...
    Upon Failure, a negative number.
*/
short  xxusb_stack_execute(libusb_device_handle *hDev, long *intbuf)
{ 
  int timeout;
    int ret;
    short lDataLen;
    unsigned char buf[26700];
    short i;
    int bufsize;
    int ii = 0;
    ...
    buf[2]=(unsigned char)(lDataLen & 255);
    lDataLen = lDataLen >> 8;
    buf[3] = (unsigned char)(lDataLen & 15);
    bufsize=intbuf[0]*2+4;
    if (intbuf[0]==0)
      return 0;
    for (i=1; i <= intbuf[0]; i++)
    ...
    if (ret>0)
		...
		if (intbuf[0]==1)
			...
			timeout=6000;
        libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_IN, buf, lDataLen, &ret, timeout);
        if (ret>0)
        for (i=0; i < ret; i=i+2)
...
******** xxusb_stack_read ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  xxusb_stack_read(libusb_device_handle *hDev, short StackAddr, long *intbuf)
{ 
  int timeout;
    int ret;
    int lDataLen;
    ...
    buf[0]=(unsigned char)(StackAddr & 51);
    buf[1]=0;
    lDataLen = 2;
    timeout=100;
    libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_OUT, buf, lDataLen, &ret, timeout);
    if (ret < 0)
        return (short) ret;
    ...
    libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_IN, buf, bufsize, &ret, timeout);
    if (ret>0)
    for (i=0; i < ret; i=i+2)
      intbuf[ii++]=(UCHAR)(buf[i]) + (UCHAR)(buf[i+1])*256;
    return (short) ret;     
...
{
  int ret;
  unsigned char buf[2] = {(unsigned char)255,(unsigned char)255}; 
  int lDataLen=2;
  int timeout=10;
  libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_OUT, buf,lDataLen, &ret, timeout);
  return (short)ret;
...
short  xxusb_devices_find(xxusb_device_type *xxdev)
{
  ...
  while((dev = devs[i++]) != NULL) {
    ...
    if (desc.idVendor == XXUSB_WIENER_VENDOR_ID) {
      libusb_open(dev,&udev);
      if (udev) {
        libusb_get_string_descriptor_ascii(udev,desc.iSerialNumber,string, sizeof(string));
...
libusb_device_handle*  xxusb_device_open(libusb_device *dev)
{
    ...
    libusb_claim_interface(udev,0);
// RESET USB (added 10/16/06 Andreas Ruben)
    xxusb_register_write(udev, 10, 0x04);
    return udev;
}
...
short  xxusb_flash_program(libusb_device_handle *hDev, char *config, short nsect)
{
  ...
  char *pconfig;
	unsigned char *pbuf;
	pconfig=config;
	unsigned char buf[518] ={0xAA,0xAA,0x55,0x55,0xA0,0xA0};
	while (*pconfig++ != -1);
	for (i=0; i<nsect; i++)
	{
		pbuf=buf+6;
		...
		libusb_bulk_transfer(hDev, XXUSB_ENDPOINT_OUT, buf, 518, &ret, 2000);
		if (ret<0)
			return ret;
...
******** VME_read_16 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  VME_read_8(libusb_device_handle *hdev,short Address_Modifier, long VME_Address, long *Data)
{
    long intbuf[1000];
    short ret;
    intbuf[0]=5;
...
******** VME_BLT_read_32 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  VME_BLT_read_32(libusb_device_handle *hdev, short Adress_Modifier, int count, long VME_Address, long Data[])
{
    long intbuf[1000];
    short ret;
    int i=0;
...
******** VME_DGG ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  VME_DGG(libusb_device_handle *hdev, unsigned short channel, unsigned short trigger, unsigned short output,
...
******** VME_scaler_settings ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  VME_scaler_settings(libusb_device_handle *hdev, short channel, short trigger, int enable, int reset)
{
	long Data, W_Data;
	short ret;
...
******** CAMAC_DGG ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  CAMAC_DGG(libusb_device_handle *hdev, short channel, short trigger, short output,
...
******** CAMAC_Output_settings ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  CAMAC_Output_settings(libusb_device_handle *hdev, int output, int code, int invert, int latch)
{
    short ret;
    long internal;
    long Data, W_Data;
...
******** CAMAC_scaler_settings ************************
  ...
  Returns:
    ...
    Upon failure, a negative number
*/
short  CAMAC_scaler_settings(libusb_device_handle *hdev, short channel, short trigger, int enable, int reset)
{
	long Data, W_Data;
	short ret;
...
******** CAMAC_block_read16 ************************
  ...
  Returns:
    ...
    Upon failure, a negative number   
*/
short  CAMAC_blockread16(libusb_device_handle *hdev, int N, int A, int F, int loops, int *Data)
{
  long intbuf[4096];  
  int i, ret =-1;
	*Data=0;
